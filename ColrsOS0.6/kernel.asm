...
.matrix_loop:
    ...
    ; 2. Отримуємо випадкову позицію (Стовпчик 0-79)
    mov ah, 0x00
    int 0x1A
    mov ax, dx
    xor dx, dx
    ...
    ; 3. Отримуємо випадковий рядок (0-24)
    mov ah, 0x00
    int 0x1A
    mov ax, dx
    rol ax, 4
    ...
    ; 4. Встановлюємо курсор у випадкову позицію
    mov ah, 0x02
    xor bh, bh
    int 10h

    ; 5. Отримуємо випадкову цифру для малювання
    mov ah, 0x00
    int 0x1A
    mov ax, dx
    and al, 0x0F
...
.is_digit:
    ...
    ; 6. Малюємо символ яскраво-зеленим
    mov ah, 0x09
    mov bl, 0x0A
    mov cx, 1
    int 10h

    ; 7. Невелика затримка, щоб очі бачили символи
    mov cx, 0x0600
.delay:
    loop .delay
...
logoline1  db "       ^          ColrsOS v0.6", 13, 10, 0
...
logoline1_anim3 db "       ^          ColrsOS v0.6", 13, 10, 0
...
helpmsg    db " Commands: start, help, clear, echo, sysinfo, animalgame, color, hacker -s, exit", 13, 10, 0
echomsg    db " Enter text: ", 0
sysinfomsg db " ColrsOS v0.6 (x86 16-bit)", 13, 10, 0
